...
public class DataInfo implements IDataInfo {
	 ...
	 *            the sample rate.
	 * @param samples
	 *            the samples. This should either be an array of integer values
	 *            or an object implementing the 'ISamples' interface.
	 * @param numSamples
	 *            the number of samples.
	 * @throws IllegalArgumentException
	 *             if the samples are invalid.
	 */
		...
		if (numSamples != 0) {
			if (samples == null) {
				...
				if (numSamples > sa.length) {
					throw new IllegalArgumentException("number of samples ("
							+ numSamples + ") is larger than data length ("
							+ sa.length + ")");
				}
