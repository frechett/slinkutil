...
/**
 * Class MiniSeedMsgHldr holds a miniSEED message and its envelope
 * information.
 */
public class MiniSeedMsgHldr extends Number {
   ...
   * @param staChaNetLoc station, channel, network, location.
   * @param startTime message start time.
   * @param messageDataArray byte array containing message data.
   * @param numSamples the number of samples.
   * @param messageNumber the SeedLink message number.
   */
  public MiniSeedMsgHldr(IStaChaNetLoc staChaNetLoc, SeedTime startTime,
      byte[] messageDataArray, int numSamples, IMessageNumber messageNumber) {
  ...
  public MiniSeedMsgHldr(String stationIdStr, String channelIdStr,
    ...
    this(new StaChaNetLoc(stationIdStr, channelIdStr, networkCodeStr,
        locationIdStr), startTime, messageDataArray, numSamples, messageNumber);
  }

  /**
   * Returns the message start time, in milliseconds since 1/1/1970.
   * This method is needed to extend the 'Number' class, which facilitates
   * using this class with a comparator.
  ...
  public String toString() {
    return staChaNetLoc + ", msgNum=" + messageNumber + ", start=" + startTime
        + ", samples=" + numSamples;
  }
  ...
  public File getFile(File outputDir) {
    return new File(outputDir, getStaChaNetLocObj().toString() + "."
