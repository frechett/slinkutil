...
public class Steim2 {
  /** The debug flag, true to display debug information, false otherwise. */
  ...
  public static int[] decode(byte[] b, int numSamples, boolean swapBytes,
    ...
    if (b.length % 64 != 0) {
      throw new SteimException(
          "encoded data length is not multiple of 64 bytes (" + b.length + ")");
    }
    ...
    if (debug != null) {
      debug.println("number of samples: " + numSamples + ", number of frames: "
          + numFrames + ", byte array size: " + b.length);
    }
    for (int i = 0; i < numFrames; i++) {
      ...
      if (i == 0) { // special case for first frame
        ...
        if (debug != null) {
          debug.println("frame " + i + ", bias = " + bias + ", x(0) = " + start
              + ", x(n) = " + end);
        }
      ...
      for (int j = firstData; j < tempSamples.length && current < numSamples; j++) {
        ...
        if (debug != null) {
          debug.print("d(" + (j - firstData) + ")" + tempSamples[j] + ", x("
   ...
   */
  public static SteimFrameBlock encode(int[] samples, int frames, int bias,
      int samplesLength) throws SteimException {
    if (samplesLength == 0) {
    ...
    if (frames > 63) {
      throw new SteimException(
          "cannot encode more than 63 frames, you asked for " + frames);
    }
    ...
    frameBlock.addEncodedWord(samples[samplesLength - 1], 0, 0); // X(N) -- last
                                                                 // sample value
    //
    ...
    int[] minbits = new int[7]; // store min # bits required to represent diff
                                // here
    ...
    while (sampleIndex < samplesLength) {
      ...
      // Determine the packing required for the next compressed word in the
      // SteimFrame.
      ...
      case 30:
        ...
        break;
      default:
        throw new SteimException("Unable to encode " + nbits
            + " bit difference in Steim2 format");
      }
      ...
      if (frameBlock.addEncodedWord(word, ndiff, nibble)) {
        ...
        // so modify reverse integration constant to be the very last value
        // added
  ...
  private static int bitsForPack(int[] minbits, int points_remaining) {
    if (points_remaining >= 7 && (minbits[0] <= 4) && (minbits[1] <= 4)
        && (minbits[2] <= 4) && (minbits[3] <= 4) && (minbits[4] <= 4)
        && (minbits[5] <= 4) && (minbits[6] <= 4))
      return 4;
    if (points_remaining >= 6 && (minbits[0] <= 5) && (minbits[1] <= 5)
        && (minbits[2] <= 5) && (minbits[3] <= 5) && (minbits[4] <= 5)
        && (minbits[5] <= 5))
      return 5;
    if (points_remaining >= 5 && (minbits[0] <= 6) && (minbits[1] <= 6)
        && (minbits[2] <= 6) && (minbits[3] <= 6) && (minbits[4] <= 6))
      return 6;
    if (points_remaining >= 4 && (minbits[0] <= 8) && (minbits[1] <= 8)
        && (minbits[2] <= 8) && (minbits[3] <= 8))
      return 8;
    if (points_remaining >= 3 && (minbits[0] <= 10) && (minbits[1] <= 10)
        && (minbits[2] <= 10))
      return 10;
   ...
   */
  private static int steimPackWord(int[] diff, int nbits, int ndiff,
      int bitmask, int submask) {
    int val = 0;
    int i = 0;
    for (i = 0; i < ndiff; i++) {
  ...
  protected static int[] extractSamples(byte[] bytes, int offset,
    ...
    if (debug != null) {
      debug.println("extractSamples: " + bytes.length + " bytes, offset is "
          + offset + ", byte swap is " + swapBytes + ", nibbles is "
          + toHexString(nibbles, 8));
    }
    int currNibble = 0;
    int dnib = 0;
    ...
    for (int i = 0; i < 16; i++) {
      ...
      case 0:
        ...
        if (offset == 0) {
          ...
          temp[currNum++] = Utility.bytesToInt(bytes[offset + (i * 4)],
              bytes[offset + (i * 4) + 1], bytes[offset + (i * 4) + 2],
      ...
      case 1:
        ...
        break;
      case 2:
        tempInt = Utility.bytesToInt(bytes[offset + (i * 4)], bytes[offset
            + (i * 4) + 1], bytes[offset + (i * 4) + 2], bytes[offset + (i * 4)
            + 3], swapBytes);
