...
public class Steim1 {
  ...
  public static int[] decode(byte[] b, int numSamples, boolean swapBytes,
    ...
    // of samples. swapBytes is set to true if the value words are to be byte
    // swapped. bias represents
    ...
    for (int i = 0; i < numFrames; i++) {
      ...
      for (int j = firstData; j < tempSamples.length && current < numSamples; j++) {
        ...
        // System.err.print("d(" + (j-firstData) + ")" + tempSamples[j] + ", x("
        // + current + ")" + samples[current] + ";"); // DEBUG
        current++;
      }
  ...
  */
  ...
  protected static int[] extractSamples(byte[] bytes, int offset,
    ...
    int currNum = 0;
    // System.err.print ("DEBUG: ");
    for (int i = 0; i < 16; i++) { // i is the word number of the frame starting
                                   // at 0
      // currNibble = (nibbles >>> (30 - i*2 ) ) & 0x03; // count from top to
      // bottom each nibble in W(0)
      currNibble = (nibbles >> (30 - i * 2)) & 0x03; // count from top to bottom
                                                     // each nibble in W(0)
      // System.err.print("c(" + i + ")" + currNibble + ","); // DEBUG
      // Rule appears to be:
      // only check for byte-swap on actual value-atoms, so a 32-bit word in of
      // itself
      ...
      // It would seem that the W(0) word is swap-checked, though, which is
      // confusing...
      ...
      case 2:
        ...
        for (int n = 0; n < 4; n += 2) {
          temp[currNum] = Utility.bytesToInt(bytes[offset + (i * 4) + n],
              bytes[offset + (i * 4) + n + 1], swapBytes);
          currNum++;
        }
        break;
      case 3:
